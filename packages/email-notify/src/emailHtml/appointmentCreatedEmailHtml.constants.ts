 ...
 */
export const APPOINTMENT_EMAIL_DOM_IDS = {
...
export const APPOINTMENT_EMAIL_DATA_FLOW = {
  ToPatient: 'ToPatient',
  ToDoctor: 'ToDoctor',
} as const;

export const APPOINTMENT_CREATED_EMAIL_STYLES = {
  root:
    'font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;font-size:15px;color:#111827;line-height:1.5;max-width:560px',
 ...
 */
export const APPOINTMENT_CREATED_DETAIL_ROW_HTML = `<tr data-appt-detail-row data-email-part="detail-row" style="background:{{ROW_BG}}"><td style="padding:10px 12px;font-weight:600;width:140px;border:1px solid #e5e7eb">{{LABEL}}</td><td style="padding:10px 12px;border:1px solid #e5e7eb">{{VALUE}}</td></tr>`;

const S = APPOINTMENT_CREATED_EMAIL_STYLES;

/**
 * Patient-facing HTML shell. Placeholders: {{DETAIL_ROWS}}, {{SAFE_PATIENT}}, {{SAFE_DOCTOR}}, {{APPT_ID}}, {{DASHBOARD_HREF}}
 */
export const APPOINTMENT_CREATED_HTML_TO_PATIENT = `<div id="${APPOINTMENT_EMAIL_DOM_IDS.rootToPatient}" data-email-flow="${APPOINTMENT_EMAIL_DATA_FLOW.ToPatient}" data-template="appointment-created" data-template-version="1" style="${S.root}">
  ...
  <table id="${APPOINTMENT_EMAIL_DOM_IDS.detailsTableToPatient}" data-email-part="details" style="${S.table}">{{DETAIL_ROWS}}</table>
  <p data-email-part="doctor-line" style="margin:0 0 12px">Assigned doctor: <strong>{{SAFE_DOCTOR}}</strong></p>
  <p data-email-part="reference-line" style="margin:0 0 20px">Reference: <code id="${APPOINTMENT_EMAIL_DOM_IDS.referenceCodeToPatient}" style="${S.code}">{{APPT_ID}}</code></p>
  <p data-email-part="cta" style="margin:0 0 8px"><a id="${APPOINTMENT_EMAIL_DOM_IDS.dashboardLinkToPatient}" href="{{DASHBOARD_HREF}}" style="${S.link}">Open your dashboard</a> to review or update this appointment.</p>
 ...
 */
export const APPOINTMENT_CREATED_HTML_TO_DOCTOR = `<div id="${APPOINTMENT_EMAIL_DOM_IDS.rootToDoctor}" data-email-flow="${APPOINTMENT_EMAIL_DATA_FLOW.ToDoctor}" data-template="appointment-created" data-template-version="1" style="${S.root}">
  ...
  <table id="${APPOINTMENT_EMAIL_DOM_IDS.detailsTableToDoctor}" data-email-part="details" style="${S.table}">{{DETAIL_ROWS}}</table>
  <p data-email-part="reference-line" style="margin:0 0 20px">Reference: <code id="${APPOINTMENT_EMAIL_DOM_IDS.referenceCodeToDoctor}" style="${S.code}">{{APPT_ID}}</code></p>
  <p data-email-part="cta" style="margin:0 0 8px"><a id="${APPOINTMENT_EMAIL_DOM_IDS.dashboardLinkToDoctor}" href="{{DASHBOARD_HREF}}" style="${S.link}">Open hospital dashboard</a> (same portal your patients use).</p>
