...
const EmailNotifyService_1 = require("../src/EmailNotifyService");
...
describe('EmailNotifyService', () => {
    ...
    it('sends reset password email and returns a token', async () => {
        const service = new EmailNotifyService_1.EmailNotifyService(baseConfig, new InMemoryTokenStore(), provider);
    ...
    it('rejects an expired reset token', async () => {
        const service = new EmailNotifyService_1.EmailNotifyService({ ...baseConfig, tokenTtlSeconds: -1 }, new InMemoryTokenStore(), provider);
        const { token } = await service.sendResetPassword('user-1', 'user@example.com', 'Rakesh');
        const userId = await service.validateResetToken(token);
    ...
    it('throws when neither resend config nor custom provider given', () => {
        expect(() => new EmailNotifyService_1.EmailNotifyService({
            provider: 'custom',
            fromAddress: 'noreply@example.com',
            appBaseUrl: 'https://app.example.com',
        }, new InMemoryTokenStore())).toThrow('No valid email provider configured');
    });
    it('uses custom provider when passed in constructor', async () => {
        const customProvider = {
            send: jest.fn().mockResolvedValue({ success: true }),
        };
        const service = new EmailNotifyService_1.EmailNotifyService(baseConfig, new InMemoryTokenStore(), customProvider);
        await service.sendAppointmentSave({
            toEmail: 'user@example.com',
            userName: 'Rakesh',
            appointmentId: 'A-1',
