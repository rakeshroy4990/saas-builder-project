...
import com.example.flexshell.uimetadata.api.UiMetadataPackageDto;
...
class UiMetadataControllerTest {
    ...
    void deleteRemovesDocument() throws Exception {
        when(uiMetadataService.deleteStored()).thenReturn(true);

        mockMvc.perform(delete("/api/ui-metadata"))
                .andExpect(status().isNoContent());

        verify(uiMetadataService).deleteStored();
    }
}
