...
_MARKER_PREVIEW_PAGE_RE = re.compile(r"marker-pages/p(\d+)\.")
...
def _marker_page_preview_object_key(file_hash: str, page_hint: int, ext: str) -> str:
    ext = (ext or "png").lower()
    if ext == "jpeg":
        ext = "jpg"
    if ext not in {"png", "jpg", "webp"}:
        ext = "png"
    return f"{file_hash}/marker-pages/p{page_hint}.{ext}"


def build_marker_page_preview_url(file_hash: str, page_hint: int, ext: str = "png") -> str:
    key = _marker_page_preview_object_key(file_hash, page_hint, ext)
    base = os.environ["SUPABASE_URL"].rstrip("/")
    return f"{base}/storage/v1/object/public/{BUCKET}/{key}"
...
def upload_marker_page_preview(
    file_hash: str,
    page_hint: int,
    img_bytes: bytes,
    ext: str = "png",
    ...
    try:
        s3.put_object(Bucket=BUCKET, Key=key, Body=img_bytes, ContentType=ctype)
        return build_marker_page_preview_url(file_hash, page_hint, ext)
    except Exception as e:
        logger.warning("[ImageStore] Marker page preview upload failed for %s: %s", key, e)
        return None


def delete_marker_images_for_page_range(file_hash: str, p0: int, p1: int) -> None:
    """Delete Marker figures / page previews whose key encodes PDF page_hint in [p0, p1] (0-based)."""
    s3 = _get_s3()
    prefixes = [f"{file_hash}/marker/", f"{file_hash}/marker-pages/"]
    try:
        to_delete: list[str] = []
        ...
        for prefix in prefixes:
            for page in paginator.paginate(Bucket=BUCKET, Prefix=prefix):
                for o in page.get("Contents", []) or []:
                    key = o.get("Key") or ""
                    m = _MARKER_PAGE_RE.search(key) or _MARKER_PREVIEW_PAGE_RE.search(key)
                    if not m:
                        continue
                    ph = int(m.group(1))
                    if p0 <= ph <= p1:
