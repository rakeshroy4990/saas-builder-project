...
# Multilingual Smart AI chat (detect → translate for retrieval → reply in locale).
TRANSLATION_CACHE_SIZE = max(16, int(os.getenv("TRANSLATION_CACHE_SIZE", "500")))
LANGUAGE_DETECTION_MIN_CONFIDENCE = _env_float("LANGUAGE_DETECTION_MIN_CONFIDENCE", 0.5)
EMERGENCY_BYPASS_ENABLED = _env_bool("EMERGENCY_BYPASS_ENABLED", True)
TRANSLATION_MODEL = os.getenv("TRANSLATION_MODEL", LLM_MODEL).strip() or LLM_MODEL
MULTILINGUAL_RESPONSE_MODEL = os.getenv("MULTILINGUAL_RESPONSE_MODEL", "").strip()