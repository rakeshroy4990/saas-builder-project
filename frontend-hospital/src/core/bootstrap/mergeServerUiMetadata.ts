...
function mergeComponentConfig(
  ...
  const deepMerge = (
    ...
    const merged: Record<string, unknown> = { ...localValue };
    const isMeaninglessOverride = (key: string, localEntry: unknown, remoteEntry: unknown): boolean => {
      if (key !== 'pageId' && key !== 'packageName') return false;
      const hasLocalValue = typeof localEntry === 'string' && localEntry.trim() !== '';
      if (!hasLocalValue) return false;
      if (remoteEntry == null) return true;
      return typeof remoteEntry === 'string' && remoteEntry.trim() === '';
    };

    for (const [key, remoteEntry] of Object.entries(remoteValue)) {
      const localEntry = merged[key];
      if (isMeaninglessOverride(key, localEntry, remoteEntry)) {
        continue;
      }
      if (isPlainObject(localEntry) && isPlainObject(remoteEntry)) {
        merged[key] = deepMerge(localEntry, remoteEntry);
      } else {
