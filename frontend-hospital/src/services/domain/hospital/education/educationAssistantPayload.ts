...
/** Replace curly quotes often emitted by LLMs so our patterns match. */
function normalizeJsonQuotes(text: string): string {
  return text.replace(/\u201c|\u201d/g, '"').replace(/\u2018|\u2019/g, "'");
}

/** Trim to outer `{ ... }` when extra text wraps the blob (e.g. markdown or logging). */
...
/**
 * Some LLM outputs use `";` before the next key instead of `",` — invalid JSON.
 * Do not use `\s*` immediately after `;` before the newline capture, or `\s*` will
 * consume the newline and the `\r?\n` branch never matches.
 */
...
function readStringArrayForFollowUps(blob: string): string[] {
  const re =
    /"(follow_up_questions|followUpQuestions|FollowUpQuestions)"\s*:\s*\[/;
  const m = re.exec(blob);
  if (!m) return [];
  let i = m.index + m[0].length;
  const out: string[] = [];
...
/** Plain text for UI; returns original string if not a recognized assistant JSON blob. */
