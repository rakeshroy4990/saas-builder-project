...
export function deriveGrowthTraitCodes(record: {
  ...
  if (bp != null && bp < 15 && (hp == null || hp >= 50)) {
    codes.push('LEAN');
  } else if (bp != null && bp > 85) {
    codes.push('STOCKY');
  } else if (wp != null && wp < 15) {
    codes.push('SLENDER');
  }
  ...
  if (!codes.some((code) => ADJECTIVE_TRAITS.has(code)) && !codes.length) {
    codes.push('NORMAL_WEIGHT');
  }
...
/** Build profile phrase from trait codes + sex — never a fixed string. */
...
/** Prefer server trait codes; always rebuild phrase from i18n when codes are present. */
