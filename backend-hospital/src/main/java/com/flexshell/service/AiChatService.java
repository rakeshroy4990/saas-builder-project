...
public class AiChatService {
    ...
    public AiChatResponse reply(String userId, AiChatRequest request, String authorizationHeader, List<String> userRoles) {
        ...
        String replyLocale = resolveReplyLocale(request);
        if (isGreetingOnly(message)) {
            LOG.info("aiChat greeting actor={} messageLength={}", actor, messageLength);
            return greetingResponse(replyLocale);
        }
        ...
        Optional<String> multilingualEmergency = safetyPolicy.detectMultilingualEmergencyResponse(message);
        if (multilingualEmergency.isPresent()) {
            return emergencyResponse(multilingualEmergency.get(), replyLocale);
        }
        if (!"expert".equalsIgnoreCase(audience) && safetyPolicy.requiresEscalation(message)) {
            ...
            return new AiChatResponse(
                    escalationMessage,
                    true,
                    escalationType.name().toLowerCase(Locale.ROOT),
                    List.of(),
                    null,
                    null,
                    List.of(),
                    ...
                    null,
                    false,
                    escalationMessage.contains("108")
            );
        }
        List<AiChatMessageDto> history = request.history() == null ? List.of() : request.history();
        LOG.info("aiChat request actor={} messageLength={} historyCount={} replyLocale={}", actor, messageLength, history.size(), replyLocale);
        ...
        PdfRagQueryAdapter.RagQueryResult ragResult = pdfRagQueryAdapter.query(
                ...
                replyLocale);
        return toAiChatResponse(ragResult, audience, message, replyLocale);
    }
    ...
    public StreamingResponseBody streamReply(String userId, AiChatRequest request, String authorizationHeader, List<String> userRoles) {
        ...
        if (isGreetingOnly(message)) {
            ...
            AiChatResponse r = greetingResponse(replyLocale);
        ...
        Optional<String> multilingualEmergency = safetyPolicy.detectMultilingualEmergencyResponse(message);
        if (multilingualEmergency.isPresent()) {
            AiChatResponse r = emergencyResponse(multilingualEmergency.get(), replyLocale);
            return out -> writeNdjsonComplete(out, r);
        }
        if (!"expert".equalsIgnoreCase(audience) && safetyPolicy.requiresEscalation(message)) {
        ...
        List<AiChatMessageDto> history = request.history() == null ? List.of() : request.history();
        String conversationId = resolveConversationId(actor, request.conversationId());
        LOG.info("aiChat stream request actor={} messageLength={} historyCount={} replyLocale={}", actor, messageLength, history.size(), replyLocale);
        return outputStream -> {
    ...
    private AiChatResponse greetingResponse(String replyLocale) {
        String greetingReply = messages.get("ai.chat.greeting", replyLocale)
    ...
    private AiChatResponse emergencyResponse(String emergencyMessage, String replyLocale) {
        return new AiChatResponse(
                emergencyMessage,
                true,
                "emergency",
                ...
                replyLocale,
                null,
                replyLocale != null && !"en".equalsIgnoreCase(replyLocale),
                true
        );
    }

    private AiChatResponse toAiChatResponse(
        ...
        boolean emergency = ragResult != null && ragResult.emergencyCall108();
        String safeReply = emergency || "expert".equalsIgnoreCase(audience)
                ? rawReply.trim()
                : safetyPolicy.enforceSafeResponse(rawReply, message);
        LOG.info("aiChat response replyLength={} detectedLocale={}", safeReply.length(), ragResult == null ? replyLocale : ragResult.detectedLocale());
        ...
        String detected = ragResult == null || ragResult.detectedLocale() == null ? replyLocale : ragResult.detectedLocale();
        return new AiChatResponse(
                safeReply,
                emergency,
                mode,
                ...
                detected,
                ragResult == null ? null : ragResult.answerEnglish(),
                ragResult != null && ragResult.showTranslationToggle(),
                emergency
        );
    }
    ...
    private void writeTransformedNdjsonLine(
        ...
        AiChatResponse response = toAiChatResponse(rag, audience, userMessage, replyLocale);
    ...
    private String resolveReplyLocale(AiChatRequest request) {
        String explicit = Objects.toString(request.replyLocale(), "").trim().toLowerCase(Locale.ROOT);
        if (!explicit.isBlank()) {
            return SupportedLocale.normalize(explicit);
        }
        HttpServletRequest servletRequest = currentRequest();
        if (servletRequest != null) {
            return RequestLocaleAttributes.readResolvedLocale(servletRequest);
        }
